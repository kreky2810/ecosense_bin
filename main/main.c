...
// ESP-DL hederi (ostatak ćemo dodati kad sve linka):
...
void app_main(void)
{
    ...
    //ESP_ERROR_CHECK(ai_init("/spiffs/pvc_classifier.espdl")); // interim stubs
    ESP_ERROR_CHECK(ai_init(NULL)); // interim stubs
...
static void camera_video_frame_operation(uint8_t *camera_buf, uint8_t camera_buf_index, uint32_t camera_buf_hes, uint32_t camera_buf_ves, size_t camera_buf_len, void *user_data)
{
    ...
    bool is_pvc = false;
    ai_process_frame(camera_buf, (int)camera_buf_hes, (int)camera_buf_ves, &is_pvc);
    if (is_pvc) {
    // TODO: otvori poklopac (GPIO/relej) i/ili prikaži status
    }
